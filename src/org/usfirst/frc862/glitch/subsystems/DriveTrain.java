...
public class DriveTrain extends Subsystem {
    ...
    public double getError() {
        return (Math.abs(left1.getClosedLoopError(0)) +
                Math.abs(right1.getClosedLoopError(0))) / 2.0;
    }
...
//    }
    ...
    public DriveTrain() {
        ...
        eachMotor((baseMotorController) -> {
            baseMotorController.configNominalOutputForward(0.1, Constants.TALON_TIMEOUT);
            baseMotorController.configNominalOutputReverse(0.1, Constants.TALON_TIMEOUT);
...
//    }
    ...
    public void setVelocity(double left, double right) {
        ...
        if (Timer.getFPGATimestamp() < slowUntil) {
            ...
            double vel = getAverageSpeed() * Constants.slowDownRate;
            checkReverse(vel,vel);
            left1.set(ControlMode.Velocity, vel);
            right1.set(ControlMode.Velocity, vel);
        } else {
    ...
    private void configureVelocityPID() {
        eachMaster((m) -> {
            ...
            m.configOpenloopRamp(Constants.openLoopRamp, Constants.TALON_TIMEOUT);
            m.configClosedloopRamp(Constants.closedLoopRamp, Constants.TALON_TIMEOUT);
            m.configMotionAcceleration((int)Constants.PHYSICAL_MAX_HIGH_SPEED_TICKS, Constants.TALON_TIMEOUT);
            m.configMotionCruiseVelocity((int) Constants.PHYSICAL_MAX_HIGH_SPEED_TICKS * 10, Constants.TALON_TIMEOUT);
        });
    }

    public void updateMotionMagic(double left, double right) {
        Logger.debug("Update motion magic: " + left + ", " + right);
        left1.set(ControlMode.MotionMagic, left);
        right1.set(ControlMode.MotionMagic, right);
    }
}
