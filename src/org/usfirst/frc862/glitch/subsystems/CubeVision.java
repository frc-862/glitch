...
public class CubeVision extends Subsystem {
	...
	private boolean visionInitialize = false;
...
//		sbuffer = new BufferedReader(new SerialReader(serialIn));
			...
			visionInitialize = true;
		} catch (RuntimeException err) {
			Logger.error("Unable to initialize vision:" + err);
		}
	}
			...
			if (inData.indexOf("=") != -1) {
				...
				if (numObjects > 0 && lastFrame.endsWith("]\r\n")) {
					...
					for (int i = 0; i < numObjects; i++) {
						...
						if (!(w / h >= 2.2 || h / w >= 2.2 || w >= 320 || h >= 250 || (w * h) >= 40000 || m > 3))
    ...
    public double[] getBestCube() throws CubeNotFoundException {
    	if (!visionInitialize) return null;
